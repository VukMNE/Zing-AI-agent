...
def solve_move(talon, move, whose_move, my_cards, opp_cards, my_taken_cards, opp_taken_cards, my_points, opp_points):
    ...
    if len(talon) == 0:
        talon.append(card_played)
    elif len(talon) == 1:
        if talon[0].value != card_played.value:
            talon.append(card_played)
            if card_played.value == 'J':
                # print('debug 1')
                collect_cards(talon, whose_move, my_taken_cards, opp_taken_cards, my_points, opp_points)
    else:
        if (talon[-1].value == card_played.value) or (card_played.value == 'J'):
            # print('debug 2')
...
def collect_cards(talon, whose_move, my_taken_cards, opp_taken_cards, my_points, opp_points):
    print('COLLECTION HAPPENS')
    print_cards_inline(talon)
    if len(talon) == 2:
        # check if it is a zing, check if both cards have the same value
        if talon[0].value == talon[1].value:
            # it is a zing
            zing_points = 10
            if talon[1].value == 'J':
                zing_points = 20
    ...
    transfer_cards_and_points(talon, whose_move, my_taken_cards, opp_taken_cards, my_points, opp_points)

def transfer_cards_and_points(talon, whose_move, my_taken_cards, opp_taken_cards, my_points, opp_points):
    if whose_move == 1:
        for i, t in enumerate(talon):
            my_points += count_points_from(talon)
            my_taken_cards.append(t)
            talon.pop(i)
    else:
        for i, t in enumerate(talon):
            opp_points += count_points_from(talon)
...
if __name__ == '__main__':
    ...
    # talon is the table at which we trow cards
    ...
    while round <= 6:
        while len(my_cards) + len(opponent_cards) > 0:
            if who_is_first == 1:
                ...
                my_move = int(input()) - 1
                solve_move(talon, my_move, 1, my_cards, opponent_cards, my_taken_cards, opp_taken_cards, my_points, opp_points)
                ...
                solve_move(talon, opp_move, 2, my_cards, opponent_cards, my_taken_cards, opp_taken_cards, my_points, opp_points)
                print('-------------------------------------------------')
                print('Now the talon is:')
                print_cards_inline(talon)
